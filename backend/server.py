...
def call_bedrock(conversation: List[Dict], user_message: str) -> str:
    ...
    for msg in conversation[-20:]:  # Last 20 messages (10 back-and-forth exchanges)
        messages.append({
            "role": msg["role"],
    ...
    try:
        ...
        # The system parameter should be a list of content blocks
        system_content = [{"text": prompt()}]
        
        response = bedrock_client.converse(
            modelId=BEDROCK_MODEL_ID,
            messages=messages,
            system=system_content,  # Use system parameter for system prompt (proper format)
            inferenceConfig={
                "maxTokens": 2000,
                "temperature": 0.7,
                "topP": 0.9
